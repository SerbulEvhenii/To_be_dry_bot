...
def show_current_daily_weather(user_id):
    temp_min = round((get_weather(user_id))['daily'][0]['temp']['min'], 0)  # [0] - сегодня, 1 - завтра
    temp_max = round((get_weather(user_id))['daily'][0]['temp']['max'], 0)  # [0] - сегодня, 1 - завтра
    pop = int((get_weather(user_id))['daily'][0]['pop'] * 100)  # Вероятность осадков % (Precipitation)
...
def show_tomorrow_weather(user_id):
    temp_min = round((get_weather(user_id))['daily'][1]['temp']['min'], 0)  # [1] - сегодня, 2 - завтра
    temp_max = round((get_weather(user_id))['daily'][1]['temp']['max'], 0)  # [1] - сегодня, 2 - завтра
    pop = int((get_weather(user_id))['daily'][1]['pop'] * 100)  # Вероятность осадков % (Precipitation)
